...
public class BanSub extends Command {
    ...
    public BanSub(TebexHandler handler) {
        ...
        Argument<String> ip = ArgumentType.String("ip").setDefaultValue("");

        addConditionalSyntax((s, str) -> {
            return s.hasPermission("minestom.store.ban");
        }, (s, c) -> {
            if(!handler.isSetup()) throw new RuntimeException("Plugin not setup.");
            try {
                boolean success = handler.getSDK().createBan(c.get(playerArg), c.get(ip), c.get(reason)).get();
                if(success)
